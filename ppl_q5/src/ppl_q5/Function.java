 ...
 * <p> It provides all the method for retrieving boy data,girl data and assign gf and bf  </p>
...
public class Function {
    ...
    /**
     * <p>This method takes array of Boy Object and array of girl object and assign girlfriend and boyfriend alternatively between boys girl</p>
            ...
            if(mod%2 == 0){
                ...
                if(l<b.size()){
                    for(int k=g.size();k>=0;k++){
                        if(g.get(k).status == -1 && g.get(k).check(b.get(l))==1 && b.get(l).status ==-1 && b.get(l).check(g.get(k))==1){
                                     Couple c = new Couple();
                                    c.b = b.get(l);
                                    c.g = g.get(k);
                                    c.b.status = k;
    ...
    /**
     * <p>sorts the array of Girl object according to attractiveness</p>
     * @param c Array list of Girl object
     */
    public void sort(ArrayList<Girl> c ){
        int i,j;
        int n = c.size();
        Girl temp;
            ...
            for(i=0;i< n-1;i++){
                for(j=0;j< n-i-1;j++){
                    if(c.get(j).attraction < c.get(j+1).attraction ){
                       temp = c.get(j);
                       c.add(j, c.get(j+1));
                       c.add(j+1, temp);
